var MaxMapMarkers = (function() {
    var marker_array = { //private list of icons, not meant for direct access
    ...
    var getMarker = function(name, color) {
        if (name == 'default' || !name) {
            if (!color) {
                return marker_array.default;
            } else {
                return getReplaceDefaultColor(color, '#4dea51');
            }
        } else {
            return getCustomMarker(name);
        }
    }
    var getReplaceDefaultColor = function(newcolor, defaultcolor) {
        if (!marker_array.color[newcolor]) { //only do this once for each color each load
            var ret = marker_array.default.options.html.replace(defaultcolor, newcolor);
            marker_array.color[newcolor] = new L.divIcon({
                html: ret,
                iconSize: marker_array.default.options.iconSize,
                iconAnchor: marker_array.default.options.iconAnchor,
                className: marker_array.default.options.className,
            });
        }
        return marker_array.color[newcolor];
    }

    var getCustomMarker = function(name) {
        if (!marker_array[name]) {
            marker_array[name] = new L.Icon({
